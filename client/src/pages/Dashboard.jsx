...
//
// ✨ New components:
//    - Stats summary row (total / upcoming / completed / certificates)
//    - Tab filter (All / Upcoming / Completed)
//    - Certificate readiness indicator with animated ring
//    - Toast instead of alert()
//    - Richer card layout with timeline-style registered date
...
function StatCard({ value, label, accent, delay }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
        {label}
      </div>
    </motion.div>
  );
...
function CertRing({ canDownload, isAttended, isCompleted }) {
  ...
  const color = canDownload ? "#10b981" : isCompleted ? "#f59e0b" : "rgba(255,255,255,0.15)";
  ...
  return (
    <svg width="44" height="44" viewBox="0 0 44 44" style={{ flexShrink: 0 }}>
      ...
      />
      <text x="22" y="26" textAnchor="middle" fontSize="11" fill={canDownload ? "#10b981" : "rgba(255,255,255,0.3)"}>
        {canDownload ? "✓" : "📜"}
      </text>
    </svg>
  );
...
// ─── Registration card ─────────────────────────────────────────────────────────
function RegCard({ reg, index, onDownload, isDownloading }) {
  ...
  const pay = payColors[reg.paymentStatus] || payColors.pending;
    ...
    >
      ...
      <div style={{
        ...
        background: isCompleted
          ? canDownload ? "linear-gradient(90deg,#10b981,#059669)" : "rgba(255,255,255,0.1)"
      ...
      <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: "14px", flex: 1 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              ...
              <span style={{
                ...
                background: isCompleted ? "rgba(100,100,100,0.15)" : "rgba(16,185,129,0.12)",
                color: isCompleted ? "rgba(255,255,255,0.35)" : "#6ee7b7",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
          {[
            ...
            <div key={text} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "12px", width: "16px", textAlign: "center", opacity: muted ? 0.4 : 1 }}>{icon}</span>
              <span style={{ fontSize: "13px", color: muted ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.5)" }}>{text}</span>
            </div>
          ))}
        </div>

        {/* Registered on — timeline style */}
        <div style={{
        ...
        >
          {isDownloading  ? "Downloading…"
            ...
            : isCompleted && !isAttended ? "Requires attendance mark"
...
export default function Dashboard() {
  ...
  // Decode name from JWT (without a library)
    ...
    <div style={{
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "36px" }}>
          <StatCard value={total}     label="Registered"   accent="white"    delay={0}    />
          <StatCard value={upcoming}  label="Upcoming"     accent="#a78bfa"  delay={0.08} />
          <StatCard value={completed} label="Completed"    accent="#6ee7b7"  delay={0.16} />
          <StatCard value={certs}     label="Certificates" accent="#fcd34d"  delay={0.24} />
        </div>
            ...
            return (
              <button key={tab} onClick={() => setTab(tab)} style={{
                ...
                <span style={{ marginLeft: "6px", fontSize: "11px", opacity: 0.6 }}>
                  {tab === "All" ? total : tab === "Upcoming" ? upcoming : completed}
                </span>
              </button>
            );
          })}
        ...
        <AnimatePresence mode="wait">
          {registrations.length === 0 ? (
            ...
            >
              ...
              <p style={{ fontSize: "18px", color: "rgba(255,255,255,0.4)", marginBottom: "8px" }}>
                No registrations yet
              </p>
              <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.2)", marginBottom: "28px" }}>
            ...
            <motion.div
              key="tab-empty"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ textAlign: "center", padding: "60px 24px", color: "rgba(255,255,255,0.25)", fontSize: "14px" }}
            >
